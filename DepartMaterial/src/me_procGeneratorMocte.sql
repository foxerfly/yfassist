USE [fanski]
GO
/****** Object:  StoredProcedure [dbo].[me_procGeneratorMocte]    Script Date: 10/17/2013 16:57:33 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
   ...
   SELECT @ZHU=ISNULL(A.MF004,'') ,@BM=ISNULL(A.MF007,'') ,@GZZX=ISNULL(MD001,'') 
   FROM (SELECT DISTINCT MF001,MF004,MF007 FROM ADMMF) AS A
   LEFT JOIN CMSMD ON MD015=MF007
   WHERE A.MF001=@UID
    ...
    WHILE @@FETCH_STATUS = 0
		BEGIN
			...
			INSERT INTO #TEMPMOCTB
			SELECT TB001,TB002, dbo.fn_me_GetMoctbCapable(TB001,TB002,TB003) TTE,TA009
			FROM  MOCTB
			LEFT JOIN MOCTA ON TB001=TA001 AND TB002=TA002
			WHERE TA030='1' AND TA013='Y' AND TA011<>'Y' AND TA011<>'y' AND TA011<>'1' AND EXISTS (SELECT TC004+TC005 FROM SFCTC WHERE TC004=MOCTA.TA001 AND TC005=MOCTA.TA002 AND TC022='Y')
			AND TB003=@PH  
			ORDER BY TA009
			
			--AND SUBSTRING(MOCTA.TA001,1,2)='51' --AND (TB004-TB005)-ISNULL(TE005,0) >0
		...
		BEGIN
					...
					END
						...
						SELECT 'fanski',@USERNAME,@ZHU,SUBSTRING(CONVERT(VARCHAR(8),GETDATE(),112),1,8),'','',1,@BLDB,@BLDH ,right(replicate('0',4)+ltrim(ROW_NUMBER() OVER (ORDER BY NEWID())),4) as 序号,
								...
								#V_MOCTE.TA001,#V_MOCTE.TA002,'','','','1',MB002,MB003,'N','',0,'','',0,'****',MB004,#V_MOCTE.VALUE,'','','',0,0,0,2,'N',
